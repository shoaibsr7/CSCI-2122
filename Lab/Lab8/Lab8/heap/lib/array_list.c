...
bool alist_swap(ArrayList* alist, int index_a, int index_b)
{
	if(alist == NULL)
		return false;
	if(index_a == index_b)
		return true;
	if(index_a >= alist->size || index_b < 0)
		return false;
	if(index_b >= alist->size || index_a < 0)
		return false;

	void* temp = alist->arr[index_a];
	alist->arr[index_a] = alist->arr[index_b];
	alist->arr[index_b] = temp;
